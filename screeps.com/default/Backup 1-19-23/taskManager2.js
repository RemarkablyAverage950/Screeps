 ...
 * @param {Room} room 
 */
function taskManager(room, roomCreeps) {
    setCreepStats(roomCreeps)
    let taskNeedingCreeps = roomCreeps.filter(c => c.memory.needTask == true)
...
function findMinerTask(creep) {
    ...
    const sources = creep.room.find(FIND_SOURCES).filter(s => {
        const noMinerAssigned = !s.assignedCreeps().some(c => Game.creeps[c].memory.role == 'miner')
        let nearContainer = false
        for (let container of containers) {
            if (s.pos.isNearTo(container)) {
                nearContainer = true
            }
        }
        return noMinerAssigned & nearContainer && s.energy >= creep.store.getFreeCapacity(RESOURCE_ENERGY)
    })
    if (sources.length > 0) {
        const target = creep.pos.findClosestByRange(sources)
        creep.memory.pending = new Task(target.id, 'harvest', RESOURCE_ENERGY)
