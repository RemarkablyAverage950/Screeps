...
function moveCreep(creep, target, range = 1, maxRooms = 1) {
    ...
    if (storedPath.length === 0) {
        try {
            ...
            storedPath = getCreepPath(creep, target, range, maxRooms).path.filter(p => p.roomName == creep.room.name)

        } catch (e) {
            console.log('Error on getCreepPath',creep.name, e)
...
module.exports = { moveCreep: moveCreep,
