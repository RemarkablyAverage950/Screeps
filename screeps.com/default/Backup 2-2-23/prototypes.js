...
Source.prototype.assignedCreeps = function () {
    return Object.values(Game.creeps).filter(c => c.memory.target == this.id || c.memory.harvestTarget == this.id).map(c => c.name)
};

Structure.prototype.assignedCreeps = function () {
    return Object.values(Game.creeps).filter(c => c.memory.target == this.id || c.memory.moveTarget == this.id).map(c => c.name)
};
