...
function buildRamparts(room) {
    let sites = Game.rooms[room].find(FIND_MY_STRUCTURES).filter(s => s.structureType == STRUCTURE_TOWER || s.structureType == STRUCTURE_SPAWN || s.structureType == STRUCTURE_STORAGE)

    for (let s of sites) {
        let ramp = s.pos.lookFor(LOOK_STRUCTURES).filter(s => s.structureType == STRUCTURE_RAMPART)
        let rampSite = s.pos.lookFor(LOOK_CONSTRUCTION_SITES).filter(s => s.structureType == STRUCTURE_RAMPART)
        if (ramp.length == 0 && rampSite.length == 0) {
            let x = s.pos.x
            let y = s.pos.y
            console.log('Placing rampart at (' + x + ',' + y + ')')
            Game.rooms[room].createConstructionSite(x, y, STRUCTURE_RAMPART)
        }
...
function buildMinableContainer(room) {
...
function autoBuild(room) {
    ...
    if (!spawns.some(s => s.room.name == room)) {
    ...
    while (!build) {
        for (let i = -range; i < range; i++) {
            for (let j = -range; j < range; j++) {
                ...
                let area = Game.rooms[room].lookForAt(LOOK_TERRAIN, checkY - 0, checkX - 0, checkY + 0, checkX + 0, true)
                for (let obj of area) {
                ...
                area = Game.rooms[room].lookForAtArea(LOOK_STRUCTURES, checkY - 0, checkX - 0, checkY + 0, checkX + 0, true)
                for (let obj of area) {
                    if (obj.structure) {
                        build = false
                    }
...
function controllerStorage(room) {
    ...
    for (var container of containers) {
        ...
        if (container.pos.inRangeTo(controller.pos, 3)) {
    ...
    for (var site of sites) {
        if (site.pos.inRangeTo(controller.pos, 3)) {
    ...
    let path = controller.pos.findPathTo(spawns[0], {
        ignoreCreeps: true,
        ignoreRoads: true,
        swampCost: 1

    })
    Game.rooms[room].createConstructionSite(path[2].x, path[2].y, STRUCTURE_CONTAINER)


    console.log('Build container')
    //path = controller.pos.getPathTo()



}

function getClearArea(room, x, y, startRange, radius, roadsAllowed) {
    ...
    while (!build) {
        for (let i = -range; i < range; i++) {
            for (let j = -range; j < range; j++) {
                ...
                let checkX = x + i
                let checkY = y + j
                if (checkX > 49 - radius || checkX < radius || checkY > 49 - radius || checkY < radius) {
                    continue
                }
                ...
                area = Game.rooms[room].lookForAtArea(LOOK_STRUCTURES, checkY - radius, checkX - radius, checkY + radius, checkX + radius, true)
                for (let obj of area) {
