    ...
    run: function (creep) {
        ...
        if (creep.memory.refill) {
            ...
            for (let percentage = 1; percentage < 100; percentage++) {
                sites = creep.room.find(FIND_STRUCTURES).filter(s => (s.hitsMax - s.hits > 100000 && (s.hits / 300000000) * 100 < percentage) && (s.structureType == STRUCTURE_WALL || s.structureType == STRUCTURE_RAMPART))
                if (sites.length > 0) {
                    break
                }
            }
