...
const remoteWorker = {
    run: function (creep) {
        ...
        if ((task.type == 'moveToRoom' && creep.room.name == task.target)
            || (task.type == 'repair' && (creep.store.getUsedCapacity() == 0 || (Game.getObjectById(task.target) && Game.getObjectById(task.target).hits == Game.getObjectById(task.target).hitsMax)))
