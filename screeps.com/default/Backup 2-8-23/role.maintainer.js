...
const roleMaintainer = {
    run: function (creep) {
        ...
        let target = Game.getObjectById(creepTasks[home][creep.name].task.target)
        if (creepTasks[home][creep.name].task.type == 'repair'
            && (!target
        ...
        if (!creepTasks[home][creep.name].task.target) {
            ...
            } else if (creep.store.getUsedCapacity() > .5 * creep.store.getCapacity()) {
                allowWithdraw = false
            }
            let availableTasks = []
        ...
        target = Game.getObjectById(creepTasks[home][creep.name].task.target)
        if (!target) {
            clearTask(creep)
            return
        }
...
function getRepairTasks(creep, structures) {
    ...
    if (repairStructures.length == 0) {
        repairStructures = structures.filter(s => s.hits < s.hitsMax)
    }
    for (let target of repairStructures) {
